...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Immediately check if they have a hospital setup
      const hospitalRes = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/hospital/my`, {
        method: "GET",
        credentials: "include",
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-8 border border-slate-200">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-1">Sign in to manage your hospital map</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          >
            {loading ? "Verifying..." : <>Sign In <ArrowRight className="ml-2 w-4 h-4" /></>}
          </button>
        </form>
