      ...
      <div className="flex-1 relative w-full overflow-hidden z-10">
        ...
        {!scannedNode && !showScanner && (
          <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4 bg-slate-950/80 backdrop-blur-sm z-20">
            ...
            <button 
              onClick={() => setShowScanner(true)}
              className="px-8 py-3 bg-blue-600 rounded-xl font-bold shadow-2xl"
            >
              Scan QR to Start
            </button>
          </div>
            ...
            <div className="space-y-4">
              <div className="relative">
                <input 
                  ...
                  className="w-full p-4 bg-white/5 rounded-xl border border-white/10 text-white outline-none focus:border-blue-500 transition-colors"
                  onFocus={() => setIsSearching(true)}
                />
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500" size={18} />
                {isSearching && (
                  <button onClick={() => setIsSearching(false)} className="absolute right-4 top-1/2 -translate-y-1/2">
              ...
              {isSearching && (
                <div className="max-h-48 overflow-y-auto space-y-2 mt-2 custom-scrollbar">
                   {poiList.length > 0 ? poiList.map((poi) => (
                      <button 
                        key={poi.id}
                        onClick={() => { 
                     ...
                     <div className="p-4 text-center text-slate-500 text-xs italic">
                        No points loaded. Try re-scanning.
                     </div>
                   )}
                </div>
              )}
