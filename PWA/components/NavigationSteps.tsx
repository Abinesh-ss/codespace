...
export default function NavigationSteps({
  ...
  // keep stable references (NO behavior change)
  ...
  }, [onPathUpdate, onStepUpdate]);

  useEffect(() => {
    console.log("⚓ Navigation Payload Tracked:", {
      startNodeId,
      endNodeId,
      floorId,
      hospitalId,
    ...
    // 🔒 prevent duplicate API calls for same route
    if (lastRequestRef.current === parametersKey) return;
    lastRequestRef.current = parametersKey;
    ...
    async function getRoute() {
      ...
      try {
        console.log("🌐 Triggering Shortest-Path Request...");
        ...
        if (data?.path?.length > 0) {
          ...
          for (let i = 0; i < path.length - 2; i++) {
            ...
            if (diff < -30) {
              ...
              generated.push({
                textEn: TXT.en.straight,
                textTa: TXT.ta.straight,
                type: "straight",
              });
            }
          }
          ...
          if (immediateInstruction?.textEn !== lastSpokenRef.current) {
            onStepUpdateRef.current(immediateInstruction.textEn);
            lastSpokenRef.current = immediateInstruction.textEn;
          }
        } else {
          setErrorMessage(
            lang === "ta"
              ? "வழித்தடம் கிடைக்கவில்லை"
              : "No path route coordinates found."
          );
        }
      } catch (err) {
        ...
        if (isMounted) {
          setErrorMessage(
            lang === "ta"
              ? "இணைப்புப் பிழை ஏற்பட்டது"
              : "Server route sync failure."
          );
        }
      } finally {
  ...
  }, [parametersKey, lang]); // ✅ FIXED: stable + prevents rerun loops

  const getIcon = (type: string) => {
    switch (type) {
      case "left":
        return <ArrowUpLeft className="text-blue-400" />;
      case "right":
        return <ArrowUpRight className="text-blue-400" />;
      case "end":
        return <CheckCircle2 className="text-emerald-500" />;
      default:
        return <ArrowUp className="text-slate-400" />;
    }
  };
    ...
    <div className="space-y-3 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar pb-10">
      {steps.length === 0 && (
        <p className="text-slate-500 text-xs italic text-center p-4">
          {lang === "ta"
            ? "வழிமுறைகளைக் காண ஒரு QR குறியீட்டை ஸ்கேன் செய்யவும்"
            : "Scan a QR to see steps"}
        </p>
      )}
      ...
      {steps.map((step, i) => {
        ...
        return (
          <div
            key={i}
            onClick={() => onStepUpdateRef.current(step.textEn)}
          ...
          >
            <div
              className={`p-2 rounded-xl ${
                isCurrentActiveStep ? "bg-white/20" : "bg-slate-800"
              }`}
            >
              {getIcon(step.type)}
            </div>
            ...
            <div>
              <p
                className={`text-sm font-bold ${
                  isCurrentActiveStep ? "text-white" : "text-slate-300"
                }`}
              >
